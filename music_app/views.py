...
def songpost(request, id):
    ...
    try:
        ...
        if(song_tag=='Classical' or song_tag=='Romantic'):
            ...
            morebooks = list(Book.objects.filter(genre='Fiction'))
            recommendedSongs+=moreSongs
            bookrecs += morebooks
            moreSongs = list(Song.objects.filter(tags='Romantic').exclude(song_id=id))
            morebooks = list(Book.objects.filter(genre='Romance'))
            bookrecs += morebooks
            recommendedSongs+=moreSongs
        ...
        if(song_tag=='Rock' or song_tag=='Pop' or song_tag=='Dance'):
            ...
            recommendedSongs+=moreSongs
            morebooks = list(Book.objects.filter(genre='Thriller'))
            bookrecs += morebooks
            morebooks = list(Book.objects.filter(genre='Casual'))
            bookrecs += morebooks
...
def allBooks(request):
    ...
    books2 = list(Book.objects.all())
    books3 = list(Book.objects.all())
    random.shuffle(books2)  # Shuffle the list of books
    random.shuffle(books3)
    return render(request, 'allBooks.html', {'books1': books1,'books2': books2, 'books3': books3})
...
#     books = Book.objects.all()
#     return render(request, 'thriller.html', {'books': books})

# def casual(request):
#     books = Book.objects.all()
#     return render(request, 'casual.html', {'books': books})

# def romance(request):
#     books = Book.objects.all()
#     return render(request, 'romance.html', {'books': books})

def genre_books(request, genre):
    books = Book.objects.filter(genre=genre)
    return render(request, 'genre.html', {'books': books, 'genre': genre})
